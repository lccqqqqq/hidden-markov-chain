...
def extract_validation_metrics(run_dir: Path) -> Dict[str, any]:
    """
    Extract validation metrics from available sources.

    Priority:
      1. checkpoint_validation_results.parquet (has R², LLR, checkpoint count)
      2. pca_metrics.csv (has per-layer R² and MSE — 438 runs have this)
    """
    defaults = {
        'best_mean_r2': None,
        'best_layer_r2': None,
    ...
    # Source 1: checkpoint_validation_results.parquet
    validation_file = run_dir / "checkpoint_validation_results.parquet"
    if validation_file.exists():
        try:
            df = pd.read_parquet(validation_file)
            defaults['best_mean_r2'] = df['mean_r2'].max() if 'mean_r2' in df.columns else None
            defaults['best_layer_r2'] = df['best_layer_r2'].max() if 'best_layer_r2' in df.columns else None
            defaults['final_llr_mean'] = df['llr_mean'].iloc[-1] if 'llr_mean' in df.columns else None
            defaults['final_llr_std'] = df['llr_std'].iloc[-1] if 'llr_std' in df.columns else None
            defaults['validation_checkpoints'] = len(df)
            return defaults
        except Exception as e:
            logger.warning(f"Failed to read validation parquet for {run_dir.name}: {e}")

    # Source 2: pca_metrics.csv (fallback — fills R² fields only)
    pca_file = run_dir / "pca_metrics.csv"
    if pca_file.exists():
        try:
            df = pd.read_csv(pca_file)
            if 'r2_score' in df.columns:
                defaults['best_layer_r2'] = float(df['r2_score'].max())
                defaults['best_mean_r2'] = float(df['r2_score'].mean())
        except Exception as e:
            logger.warning(f"Failed to read pca_metrics.csv for {run_dir.name}: {e}")
...
def scan_records_directory(records_dir: Path = None) -> List[Path]:
    ...
    Finds run directories (named YYYYMMDD_HHMMSS[_uuid]) at the top level
    and inside subdirectories like sweep01/.
    """
    if records_dir is None:
        records_dir = Path("/mnt/extraspace/clin/records")
    ...
    for d in records_dir.iterdir():
        if not d.is_dir():
            continue
        # Run dirs start with a date pattern (YYYYMMDD)
        if d.name[:8].isdigit() and len(d.name) >= 15:
            run_dirs.append(d)
        # Subdirectories (e.g. sweep01/) may contain run dirs
        elif not d.name.startswith('.') and d.name not in ('cached',):
            for sub in d.iterdir():
                if sub.is_dir() and sub.name[:8].isdigit() and len(sub.name) >= 15:
                    run_dirs.append(sub)
...
def main():
    ...
    if args.records_dir is None:
        args.records_dir = Path("/mnt/extraspace/clin/records")
